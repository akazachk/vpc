...
You may need to use a compatible version of `clang` or `g++` (for the inline variable features from C++17, at least version 7 of `g++` is needed), and make sure that the same compiler is used when running [`install_coin.sh`](setup/install_coin.sh) and the one used to generate `libgurobi_c++.a`, which can be rebuilt in the Gurobi directory (located at, say, `${GUROBI_HOME}`), under `${GUROBI_HOME}/src/build`.
