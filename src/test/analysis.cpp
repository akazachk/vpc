...
const int countExtraInfoEntries = 5; // hostname, ExitReason, end_time_string, time elapsed, instname
  ...
  try {
    ...
    if (partialDisj) {
      disjInfo.avg_explored_nodes +=
          (disjInfo.avg_explored_nodes * (num_disj - 1) + partialDisj->data.num_nodes_on_tree) / num_disj;
      disjInfo.avg_pruned_nodes +=
          (disjInfo.avg_pruned_nodes * (num_disj - 1) + partialDisj->data.num_pruned_nodes) / num_disj;
...
void updateGMICInfo(
    ...
    SummaryCutInfo& cutInfo,
    /// [in] Cuts generated in current round
    const OsiCuts* cuts,
    /// [in] Value of epsilon to use for checking cut density
    const double EPS) {
  const int old_num_cuts = cutInfo.num_cuts;
  cutInfo.num_cuts += cuts->sizeCuts();
  cutInfo.cutType.resize(cutInfo.cutType.size() + cuts->sizeCuts(), CglVPC::CutType::GMIC);
